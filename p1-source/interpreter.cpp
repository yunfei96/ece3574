...
list Interpreter::build_tree(vector<string> &token)
{
...
void Interpreter::eval_tree(list &input, envir &environent_set_up)
{
...
double Interpreter::string_to_double(string input)
{
    ...
    for(int i=0; i<input.size();i++)
    {
        ...
        if (is_number(input[i]) || input[i] == '+' || input[i] == '-' || input[i] == '.')
        {
            if((add <= 1) && (minus <= 1) && (dot <= 1))
            {
                continue;
            }
        }
...
void Interpreter::calculate(list &input, envir &environent_set_up, vector<Expression> args)
{
...
void Interpreter::special(list &input, envir &environent_set_up, int i)
{
    //update first node
...
list Interpreter::to_list(string tokens)
{
    list numlist;
    if(tokens == "True")
    {
